...
import * as pageData from '../../../../../assets/data/inventory/stock-count-session.json';
...
interface ScreenData {
  ...
  readonly records: readonly StockCountRecord[];
...
@Component({
  selector: 'app-stock-count-session',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './stock-count-session.html',
  styleUrl: './stock-count-session.css',
})
export class StockCountSessionComponent {
  ...
  protected readonly filteredRecords = computed(() => {
    ...
    return this.data.records.filter((r) =>
      !q || r.countSessionReference.toLowerCase().includes(q) || r.warehouseAndZone.toLowerCase().includes(q) || r.countedItemAndBatch.toLowerCase().includes(q)
    );
  });
  ...
  protected readonly owner = 'V. Sharma · Warehouse Manager';
  protected readonly operatingTimeZone = 'Asia/Kolkata · IST (UTC+05:30)';
  protected readonly lastRefresh = signal('Today, 02:30 PM · IST');

  // Detail modal
  protected readonly detailRow = signal<StockCountRecord | null>(null);

  protected openDetail(row: StockCountRecord): void {
