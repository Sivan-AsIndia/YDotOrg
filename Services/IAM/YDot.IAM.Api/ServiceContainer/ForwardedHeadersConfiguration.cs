...
///
/// The symptom was visible on the security screen: every session and every sign-in row recorded
/// <c>::ffff:172.20.0.9</c> — the address of the nginx CONTAINER — instead of the person's own.
/// Two consequences, and the first is the severe one:
///
///   - THE PER-IP SIGN-IN RATE LIMIT BECOMES GLOBAL. One apparent address for the whole platform
///     means <c>SignInAttemptsPerMinutePerIp</c> is shared by everybody, so a room full of people
///     signing in for a demonstration throttle each other out.
///   - Every audit row names the proxy, so "where did this come from" has one answer for every
///     event ever recorded — which is the same as having no answer.
///
...
/// the container's port IS published to the host (6702), so "only nginx can reach it" is not
