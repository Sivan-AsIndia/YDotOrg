...
/// byte-for-byte the same shape IAM and DON return, so the Angular interceptor written for one
/// service works for all three without a second code path.
///
...
/// the empty fields and give the caller a different set of keys on success and on failure. The
/// attribute overrides that for the envelope only: all six keys are always written. Payload
/// DTOs inside <c>Data</c> keep the global behaviour and still omit their own nulls.
/// </summary>
...
/// declared here as well, always null, so a failure produced outside a controller has exactly
/// the same six keys as one produced inside it.
/// </summary>
