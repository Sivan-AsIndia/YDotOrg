...
namespace YDots.CAM.Infrastructure.Authorization;

/// <summary>The requirement behind a <c>[HasPermission("cam.campaigns.approve")]</c> attribute.</summary>
...
/// adding a permission never needs a startup registration - which is what keeps forty-odd codes
/// from becoming forty lines of configuration.
/// </summary>
...
/// permission?" is one that will eventually answer differently from the other.
/// </summary>
...
/// endpoint with <c>[HasPermission("cam.campaigns.approve")]</c> and the policy is built the
/// first time that route is hit. The alternative is one <c>AddPolicy</c> line per code, which
/// is a list somebody eventually forgets to extend - and the failure mode there is an endpoint
/// that throws "no policy found" rather than one that refuses the caller.
/// </summary>
